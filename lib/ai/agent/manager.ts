...
export class AgentManager {
  ...
  async processRequest(request: string): Promise<void> {
    ...
    try {
      ...
      this.streamToUser(`I encountered an error while processing your request: ${error instanceof Error ? error.message : String(error)}`);
      this.state = 'idle';
    }
  }
  ...
  private async executeSteps(steps: Array<any>): Promise<ExecutionResult> {
    ...
    for (const step of steps) {
      try {
        ...
        success = false;
        this.streamToUser(`Error in step "${step.description}": ${error instanceof Error ? error.message : String(error)}`);
      }
    }
