...
export const ROUTER_SYSTEM_PROMPT = `You are an intelligent Router Agent that determines which specialized agent is best suited to handle a user's request.

Your task is to carefully analyze the user's message and select the most appropriate agent based on their capabilities:

1. ChatAgent ("chat"): General conversation, basic questions, casual interaction, creative content
2. ResearchAgent ("research"): Web search, finding current information, research tasks, fact checking
3. DocumentAgent ("document"): Creating, editing, and managing documents, emails, code generation
...
const routingDecisionSchema = z.object({
  selectedAgentId: z.enum(['chat', 'research', 'document']),
      ...
      if (fallbackAgent) {
        ...
        this.context.dataStream.writeData({
          ...
          message: {
            id: generateUUID(),
            role: 'assistant',
            parts: ['I apologize, but I\'m currently unable to process your request. Please try again later.']
          }
        });
      }
    }
